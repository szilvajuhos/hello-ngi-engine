...
def update_charon_with_local_jobs_status(quiet=False, config=None, config_file_path=None, trace=None):
    ...
    In an ideal version we are using a trace file generated by NextFlow. As these files are generated on the sample 
    ...
    Now only logging into the log file
    """
    ...
    # TODO: fix in an other fork
    #parse_NF_trace_file(trace)

    LOG.info("Log from update_charon_with_local_jobs_status() ...")
    return True
